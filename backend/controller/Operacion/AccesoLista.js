...
function AccesoLista(_instruccion, _ambito){
    ...
    if(existe){
        ...
        if (simbolo.tipo.search("LISTA") ===-1){
            return {
                valor: `Error Semantico: El tipo de dato no es un lista: ${simbolo.tipo},... Linea: ${_instruccion.linea} Columna: ${_instruccion.columna}`,
                tipo: null,
        ...
        if(posicion.valor<tam && posicion.valor>=0){
            ...
            _Error.addErrores(nuevo)*/
            return{
                valor: `Error Semantico: La posición ingresada: ${posicion.valor} no cumple con la longitud del lista... Linea: ${_instruccion.linea} Columna: ${_instruccion.columna}`,
                tipo: null,
                linea: _instruccion.linea,
                columna: _instruccion.columna
            }
    ...
    /*var nuevo=new ERRORES(TIPO_ERROR.SEMANTICO,`el vector '${String(id)}' no existe.`,_instruccion.linea, _instruccion.columna);
        _Error.addErrores(nuevo)*/
        return {
            valor: `Error Semantico: el lista '${String(id)}' no existe... Linea: ${_instruccion.linea} Columna: ${_instruccion.columna}`,
            tipo:null,
            linea: _instruccion.linea,
            columna: _instruccion.columna
        }
